...
public class TerrainControllerScreen extends AbstractButtonListScreen implements NordColors {
    ...
    public TerrainControllerScreen(BlockPos blockPos) {
        ...
        for (TerrainModifier<?> terrainModifier : TerrainModifiers.MODIFIERS_REGISTRY) {
            if (data.getStoredData().modifiers().containsKey(terrainModifier)) {
                continue;
            }
            data.getStoredData().modifiers().put(terrainModifier, new ModifierState<>(terrainModifier.isEnabledByDefault(), terrainModifier.getDefaultValue(level, biome, blockPos)));
        }
    ...
    @NotNull
    private <T> SimpleButton createSimpleButton(Panel panel, TerrainModifier<T> modifier, ModifierState<T> state) {
    ...
    public void addButtons(Panel panel) {
        data.getStoredData().modifiers().entrySet().stream()
    ...
    private class ModifierRow<T> extends Panel {
        ...
        private ModifierState<T> state;

        ModifierRow(Panel parent, TerrainModifier<T> modifier, ModifierState<T> state) {
            super(parent);
            this.searchText = Component.translatable(modifier.id().toLanguageKey());
            this.state = state;
            ...
            this.resetButton = BetterButton.simple(this, Component.literal("Settings"), Icons.SETTINGS, () -> openContextMenu(List.of(
                    new ContextMenuItem(Component.literal("Reset to Default"), Icons.REFRESH, (btn) -> {
                        this.state = state.withValue(modifier.getDefaultValue(Minecraft.getInstance().level, Minecraft.getInstance().level.getBiome(blockPos).value(), blockPos));
                        sendUpdate(modifier, this.state);
                    })
            )));
        ...
        public TerrainButtonListTopPanel() {
            ...
            this.settingsButton = BetterButton.simple(this, Component.literal("Settings"), Icons.SETTINGS, () -> openContextMenu(List.of(
                    new ContextMenuItem(Component.literal("Reset All to Default"), Icons.REFRESH, (btn) -> {
                        StoredData storedData = data.getStoredData();
                        storedData.modifiers().clear();
    ...
    private void sendUpdate(TerrainModifier<?> modifier, ModifierState<?> newState) {
        data.getStoredData().modifiers().put(modifier, newState);
        GlobalPos controllerPos = GlobalPos.of(Minecraft.getInstance().player.level().dimension(), blockPos);
        ClientPacketDistributor.sendToServer(new UpdateControllerPacket(controllerPos, data));
